...
export default function AttendanceReportTable({ onRowClick }) {
    ...
    const filteredData = activeTab === 'allZones' ? attendanceReportData : attendanceReportData.filter((report) => report.zone.replace(/\s/g, '') === activeTab);
    const totalCount = attendanceReportData.length;
    const countAdamawaSouth = attendanceReportData.filter((report) => report.zone === 'Adamawa South').length;
    const countAdamawaNorth = attendanceReportData.filter((report) => report.zone === 'Adamawa North').length;
    const countAdamawaCentral = attendanceReportData.filter((report) => report.zone === 'Adamawa Central').length;
            ...
            <div className="dashboard-attendance-table-section my-3">
                <div className="attendance-header  pt-5 pe-5">
                    ...
                    <div className="d-flex  tab-header my-4">
                        ...
                        <div
                            className={`tab-item ${activeTab === 'AdamawaSouth' ? 'active' : ''}`}
                            onClick={() => handleTabChange('AdamawaSouth')}
                        >
                            Adamawa South ({countAdamawaSouth})
                        </div>
                        <div
                            className={`tab-item ${activeTab === 'AdamawaNorth' ? 'active' : ''}`}
                            onClick={() => handleTabChange('AdamawaNorth')}
                        >
                            Adamawa North ({countAdamawaNorth})
                        </div>
                        <div
                            className={`tab-item ${activeTab === 'AdamawaCentral' ? 'active' : ''}`}
                        ...
                        <Table>
                            ...
                            <TableBody>
                                {filteredData
                                    ...
                                    .map((report, index) => (
                                        <TableRow key={report.id} onClick={() => onRowClick(report.id)}>
                                            <TableCell>{index + 1}</TableCell>
                                            <TableCell>
                                                <Link to={`/detailed-attendance/${report.id}`} key={report.id}>
                                            ...
                                            <TableCell className={isTimePastFour(report.time_sent) ? 'red-color' : ''}>
